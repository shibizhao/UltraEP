...
def run_case(manager, weights, grads, args, operation):
    ...
    def batch():
        for step in range(8):
            ...
            kwargs = {"async_finish": not args.sync}
            if args.current_stream and operation == "grad_reduce":
                kwargs["use_current_stream"] = True
            if args.previous_event:
                kwargs["previous_event"] = ultra_ep.EventHandle(
                    ultra_ep._C.EventHandle()
                )
                with torch.cuda.stream(api_stream):
                    event = getattr(manager, operation)(layer, **kwargs)
                    if not args.sync:
                        event.current_stream_wait()
                stream.wait_stream(api_stream)
            else:
                event = getattr(manager, operation)(layer, **kwargs)
                if not args.sync:
                    event.current_stream_wait()
